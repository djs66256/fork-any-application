...
| 频道 | mobile |
| 采集方案 | ADB |
| 目标页面 | mobile/homepage-feed/search/ranking/recommend-list/ai-tab |
| 采集范围 | 从推荐榜切换到顶部“AI剧”内容类型，记录 AI 剧内容池首屏结构与返回链路 |
...
*采集阶段回填：*
- **观察**：从排行页切到推荐榜后，顶部一级标签为“全部 / 真人剧 / 漫剧 / AI剧 / 演员”，默认停留在“全部”。这说明“AI剧”属于推荐榜内的内容类型切换，而非独立页面入口。
- **截图文件**：`assets/2026-07-24-step-01-recommend-list.png`

### 步骤 2：点击“AI剧”Tab

- **操作**：在推荐榜页点击顶部“AI剧”内容类型标签
- **命令**：`adb shell input tap 503 319 && sleep 2`
- **截图**：保存为 `assets/2026-07-24-step-02-ai-tab.png`
- **观察要点**：确认榜单标题、列表内容与内容类型是否切换为 AI 剧池

*采集阶段回填：*
- **观察**：点击“AI剧”后，页面标题切换为《AI剧推荐榜》，顶部一级标签高亮变为“AI剧”，榜单仍沿用推荐榜的“推荐”主指标，但内容列表明显转为更偏脑洞、玄幻、宫廷、时空、进化等想象力驱动题材，海报也更偏插画化与 AI 生成质感。
- **截图文件**：`assets/2026-07-24-step-02-ai-tab.png`

### 步骤 3：返回排行/搜索页上文

- **操作**：从 AI剧推荐榜状态执行返回，确认回到上一层
- **命令**：`adb shell input keyevent KEYCODE_BACK && sleep 2`
- **截图**：保存为 `assets/2026-07-24-step-03-back-context.png`
- **观察要点**：记录返回落点和推荐榜状态是否保留

*采集阶段回填：*
- **观察**：从 AI剧推荐榜返回后，直接回到搜索承接页，而不是停留在推荐榜默认态。这说明推荐榜中的内容类型切换仍属于排行承接层内部状态，返回操作会退出整个排行体系。
...
| `assets/2026-07-24-step-02-ai-tab.png` | 截图 | 步骤 2 | AI剧推荐榜首屏 |
...
| 14:13:08 | 切换 AI剧 Tab | `adb shell input tap 503 319 && sleep 2` | 成功，进入 AI剧推荐榜 |
