...
播放器当前仍处于跨端路由承载阶段：Web、Android、iOS 都已能根据路由参数进入播放页占位页面并展示 `videoId`，但尚未接入真实视频播放、播放控制、进度上报或后端业务逻辑。与 PRD-01 不同的是，PRD-02 之后移动端播放页的代表性入口已经从首页示例按钮切换为首页信息流卡片动作，播放链路开始依附真实首页 Feed 数据；Backend 仍预留 `POST /api/player/start` 与 `POST /api/player/stop` 两个 RESTful 路由，但当前统一返回 501。

- 核心价值：先打通播放页路由、参数透传与首页 Feed 卡片入口，为后续真实播放器能力提供稳定落点
- 覆盖范围：Web、Android、iOS、Backend
- 当前状态：页面路由与参数展示已实现，移动端入口已接入首页信息流；播放能力与接口逻辑未实现
...
| Android | 首页 Feed 卡片“播放”按钮、deeplink | `play/{videoId}`（兼容 `player/{videoId}`）、`djsdrama://play/{id}`、`djsdrama://player/{id}` | `android/app/src/main/java/com/djs66256/short_drama/feature/home/ui/HomeScreen.kt:71-76`, `android/app/src/main/java/com/djs66256/short_drama/navigation/AppDestination.kt:29-83`, `android/app/src/main/java/com/djs66256/short_drama/navigation/DeeplinkRouteParser.kt:28-35` |
| iOS | 首页 Feed 卡片“观看”按钮、deeplink | `play` public route name、`djsdrama://play/{id}` | `ios/ShortDrama/Sources/Features/Home/Views/HomeView.swift:46-56`, `ios/ShortDrama/Sources/App/AppRoute.swift:19-27`, `ios/ShortDrama/Sources/App/DeeplinkHandler.swift:23-31` |
...
1. 用户从首页入口进入播放页，或通过 deeplink 打开播放目标。
   - Web：首页仍通过代表性链接进入 `/play/sample`（`web/src/features/home/HomeScreen.tsx:36-38`）。
   - Android：首页 `HomeScreen` 卡片点击时调用 `onOpenPlay(drama.id)`（`android/app/src/main/java/com/djs66256/short_drama/feature/home/ui/HomeScreen.kt:209-218`）。
   - iOS：首页 `HomeDramaCardView` 点击“观看”时经 `HomeRouteBuilder.playerRoute(for:)` 构造 `.player(videoId: drama.id)`（`ios/ShortDrama/Sources/Features/Home/Views/HomeView.swift:46-56,98-112`）。
2. 路由层读取 `videoId` 并把它交给播放页占位 View / ViewModel。
   ...
   - iOS：`TabNavigationHostView` 在路由命中 `.player(let videoId)` 时构造 `PlayerViewModel(videoId: videoId)`（`ios/ShortDrama/Sources/App/TabNavigationHostView.swift:11-18`）。
...
| Android deeplink 使用历史 `player` host | 解析后统一映射到 `PendingRoute.Play(videoId)` | `android/app/src/main/java/com/djs66256/short_drama/navigation/DeeplinkRouteParser.kt:28-35` |
...
| 首页卡片 `id` 为空 | Android 不触发播放导航；iOS `HomeRouteBuilder.playerRoute(for:)` 返回 `nil` | `android/app/src/main/java/com/djs66256/short_drama/feature/home/ui/HomeScreen.kt:158-170,209-218`, `ios/ShortDrama/Sources/Features/Home/Views/HomeView.swift:98-112` |
...
- 路由定义：`android/app/src/main/java/com/djs66256/short_drama/navigation/AppDestination.kt:29-83`
- 导航注册：`android/app/src/main/java/com/djs66256/short_drama/navigation/NavGraph.kt:121-146`
...
- 路由定义：`ios/ShortDrama/Sources/App/AppRoute.swift:4-29`
- 导航注册：`ios/ShortDrama/Sources/App/TabNavigationHostView.swift:11-18`
...
| 首页 Feed `drama.id` | 列表项字段 | 页面级 | 作为移动端播放路由的统一参数来源 | `android/app/src/main/java/com/djs66256/short_drama/feature/home/ui/HomeScreen.kt:209-218`, `ios/ShortDrama/Sources/Features/Home/Views/HomeView.swift:98-112` |
...
| 首页信息流 | 导航入口 | 移动端真实入口现已来自首页 Feed 卡片，而非示例按钮 |

### 外部依赖

| 服务 | 用途 | 接入方式 |
|------|------|---------|
| Backend Dramas API | 提供首页卡片与 `drama.id` | `GET /api/dramas` |
...
| 问题 | 影响 | 记录时间 | 备注 |
|------|------|---------|------|
| 页面仅为占位实现 | 只能验证路由与参数透传，无法验证真实播放体验 | 2026-07-26 | 三端页面都只展示标题与 `videoId` |
| Backend 接口未实现 | 前端无法接入真实播放启动/停止流程 | 2026-07-26 | `POST /api/player/start`、`POST /api/player/stop` 都返回 501 |
| iOS 不兼容 `player` 历史 host | 旧 deeplink 兼容仅在 Android 保留 | 2026-07-26 | iOS 仅解析 `play` |
| 设备级黑盒仍待补测 | 无法确认真实首页卡片点击后播放页跳转的设备表现 | 2026-07-26 | `docs/specs/2026-07-25-prd-02-homepage-feed/qa-test.md:84-100` |
