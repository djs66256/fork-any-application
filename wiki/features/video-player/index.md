...
播放器当前处于跨端路由承载阶段：Web、Android、iOS 都已能根据路由参数进入播放页占位页面并展示 `videoId`，但尚未接入真实视频播放、播放控制、进度上报或后端业务逻辑。Backend 已预留 `POST /api/player/start` 与 `POST /api/player/stop` 两个 RESTful 路由，但当前仍统一返回 501。

- 核心价值：先打通播放页路由、参数透传与 deeplink 承载，为后续真实播放器能力提供稳定落点
- 覆盖范围：Web、Android、iOS、Backend
- 当前状态：页面路由与参数展示已实现，播放能力与接口逻辑未实现
...
| Android | 首页按钮、deeplink | `play/{videoId}`（兼容 `player/{videoId}`）、`djsdrama://play/{id}`、`djsdrama://player/{id}` | `android/app/src/main/java/com/djs66256/short_drama/navigation/AppDestination.kt:29-83`, `android/app/src/main/java/com/djs66256/short_drama/navigation/DeeplinkRouteParser.kt:28-35` |
| iOS | 首页按钮、deeplink | `play` public route name、`djsdrama://play/{id}` | `ios/ShortDrama/Sources/App/AppRoute.swift:19-27`, `ios/ShortDrama/Sources/App/DeeplinkHandler.swift:23-31` |
...
1. 用户从首页代表性入口进入播放页，或通过 deeplink 打开播放目标。
   - Web：首页链接到 `/play/sample`（`web/src/features/home/HomeScreen.tsx:36-38`）
   - Android：首页点击 `onOpenPlay("sample-play")`（`android/app/src/main/java/com/djs66256/short_drama/feature/home/ui/HomeScreen.kt:70-72`）
   - iOS：首页按钮调用 `router.navigate(to: .player(videoId: "sample-play"))`（`ios/ShortDrama/Sources/Features/Home/Views/HomeView.swift:43-47`）
...
| 问题 | 影响 | 记录时间 | 备注 |
|------|------|---------|------|
| 页面仅为占位实现 | 只能验证路由与参数透传，无法验证真实播放体验 | 2026-07-25 | 三端页面都只展示标题与 `videoId` |
| Backend 接口未实现 | 前端无法接入真实播放启动/停止流程 | 2026-07-25 | `POST /api/player/start`、`POST /api/player/stop` 都返回 501 |
| iOS 不兼容 `player` 历史 host | 旧 deeplink 兼容仅在 Android 保留 | 2026-07-25 | iOS 仅解析 `play` |
