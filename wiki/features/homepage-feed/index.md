...
- 自动化证据：`HomeViewModelTest`、`HomeScreenTest`、`RoutesTest` 覆盖状态机、卡片 meta 和路由映射；评论接线与共享卡片签名兼容性也已回归
...
- 自动化证据：`APIClientTests`、`DramaRepositoryTests`、`HomeViewModelTests`、`NavigationRouterTests` 覆盖 endpoint/query、canonical response 解码、状态机与路由映射；评论宿主状态也已在首页 ViewModel 测试中补充
...
- 自动化证据：`dramas.test.ts`、`schemas.test.ts`、`drama.mock.repository.test.ts`、`drama.service.test.ts` 覆盖 contract、schema、分页与边界行为

### Web

- 首页仍展示应用信息与代表性链接，不消费首页 Feed
- 本期设计范围明确 Web 不实现首页信息流与 comments UI
...
| 服务 / 框架 | 用途 | 接入方式 |
|-------------|------|---------|
| Next.js Route Handlers | Backend 列表接口与评论接口承载 | `backend/src/app/api/dramas/route.ts`、`backend/src/app/api/dramas/[id]/comments/*` |
| Retrofit | Android 首页拉取 Feed 与首页评论请求 | `android/app/src/main/java/com/djs66256/short_drama/core/network/ApiService.kt` |
| URLSession + APIClient | iOS 首页拉取 Feed 与首页评论请求 | `ios/ShortDrama/Sources/Data/DataSources/DramaRemoteDataSource.swift`、`ios/ShortDrama/Sources/Data/DataSources/CommentRemoteDataSource.swift` |
...
| 设备级黑盒仍待补测 | 移动端真实点击、首页评论抽屉交互、错误态和恢复时序未在设备/模拟器上执行 | 2026-07-29 | 本轮 QA 以自动化测试、构建和代码检查为主 |
