...
PRD-09 评论系统已经在当前代码中完成首版落地：Backend 已提供评论列表、发表评论、点赞 / 取消点赞三条 RESTful API；Android 与 iOS 都已把评论入口接到首页 Feed 卡片与播放器页内，并以内嵌 bottom sheet / sheet 的方式承载评论列表、排序、分页、发表评论、点赞，以及未登录写操作的拦截与恢复。评论首版遵循“匿名可读、登录可写”的产品语义，但后端已经对齐当前 canonical auth middleware：评论列表使用可选鉴权，发表评论与点赞使用强制鉴权；客户端宿主层当前只负责**拦截 + 提示 + 恢复评论容器上下文**，不会自动重放原发送或点赞动作。
...
4. 评论写操作未登录时会发出 `CommentEffect.RequireLogin(CommentLoginContext)`；当前宿主层以 Toast + placeholder dialog 承接登录提示，并在确认后回到评论抽屉。
...
3. 评论列表允许匿名读取；评论列表 route 通过 `resolveOptionalAuthContext()` 解析可选登录态，发表评论与点赞通过 `requireAuthContext()` + `getAuth(request)` 强制要求登录。
...
4. Backend 对写请求统一要求当前 canonical auth context；匿名写操作返回 401。
...
| 认证体系 | 写接口鉴权与上下文恢复 | 评论列表复用可选 auth；写接口复用强制 auth；宿主层只负责恢复评论容器上下文 |
| 排行体系 | 登录拦截模式参考 | 评论恢复语义借鉴了排行“保留上下文后返回页面”的思路，但已独立实现自己的 `CommentLoginContext` |
...
| 登录承接仍是轻量宿主方案 | Android / iOS 当前只能验证“拦截 + 恢复上下文”语义，评论页内不会直接拉起完整登录流 | 2026-07-29 | Android placeholder dialog，iOS alert |
...
| 日期 | 变更摘要 |
|------|---------|
| 2026-07-29 | 更新：根据当前 worktree 真实代码，将评论能力从“未实现”修正为“Backend + Android + iOS 首版已落地”，并同步 comments routes 已对齐当前 auth middleware、首页 / 播放器入口、评论抽屉、登录恢复上下文与已知限制 |
