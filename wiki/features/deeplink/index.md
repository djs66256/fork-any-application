...
通过自定义 URL Scheme `djsdrama://` 实现外部唤起应用并落到应用壳内部路由的能力。当前 iOS 与 Android 都已接入 deeplink 解析；其中 iOS 直接支持 `open` / `search` / `ranking` / `classification` / `new-releases` / `actors` / `play` / `drama` 等 host，Android 在此基础上额外兼容历史 `player` host，并统一映射到 canonical `play` 语义。PRD-05 之后，`ranking` 已不再是占位承接，而是可直接打开真实排行页的有效外部入口；PRD-06 之后，`classification` 也不再是占位承接，而是可直接打开真实分类页的有效外部入口。Web 不参与 `djsdrama://` 协议处理。

- **覆盖端**：iOS、Android
- **核心价值**：让外部入口可在多 Tab 应用壳下稳定打开首页、搜索发现、排行页、分类页、播放页与详情页
- **当前状态**：多端解析与导航承载已接入，`ranking` 与 `classification` host 都已成为真实功能入口；设备级外部唤起黑盒验证仍待补测
...
### Web
- 不处理 `djsdrama://` Scheme
- Web 对应的公开页面语义由 `/play/[id]` 与 `/detail/[id]` 文件系统路由承载；`/search` 与 `/rankings` 仅作为占位页存在（`web/src/app/play/[id]/page.tsx:14-39`、`web/src/app/detail/[id]/page.tsx:14-39`、`web/src/app/search/page.tsx:1-10`、`web/src/app/rankings/page.tsx:1-9`）
...
2. `DeeplinkHandler.handleDeepLink(_:)` 校验 scheme 必须为 `djsdrama`，并按 host 解析到 `AppRoute`；`ranking` 会直接解析到 `.rankingHome`，`classification` 会直接解析到 `.classificationHome`（`ios/ShortDrama/Sources/App/DeeplinkHandler.swift:13-45`）。
...
2. `DeeplinkRouteParser.parse(...)` 先校验 scheme，再读取 host/path segment，并把 `play` 与历史 `player` 都映射为 `PendingRoute.Play`；`ranking` 会映射为 `PendingRoute.Ranking`，`classification` 会映射为 `PendingRoute.Classification`（`android/app/src/main/java/com/djs66256/short_drama/navigation/DeeplinkRouteParser.kt:14-43`）。
...
4. `NavGraph` 监听 `pendingRoute`，在容器已可导航时执行 `navController.navigate(...)` 并消费队列；其中 `PendingRoute.Ranking` 会导航到默认 `ranking?contentType=all&type=hot` 页面，`PendingRoute.Classification` 会导航到真实 `classification` 页面（`android/app/src/main/java/com/djs66256/short_drama/navigation/NavGraph.kt:77-105`）。
...
### Web
- 不实现自定义 Scheme 解析。
- 公开页面语义与移动端 canonical naming 对齐：`play` / `detail`；`search` 与 `rankings` 仅保留占位页。

## API 引用

本功能不新增独立 RESTful API。deeplink 解析和路由分发均在端内完成；`search` / `ranking` / `classification` deeplink 最终承接的业务数据分别来自 [../../api/dramas.md](../../api/dramas.md) 中的 `GET /api/dramas/search`、`GET /api/dramas/rankings`、`GET /api/dramas/tags`。
...
| 分类浏览 | 路由目标 | `classification` deeplink 最终映射到真实分类页 |
...
- 当前自动化验证覆盖了解析与状态流转，但尚未完成真实设备/模拟器上的外部 App 唤起黑盒测试（见 `docs/specs/2026-07-27-prd-06-classification/qa-test.md:24-25,305-313`）。
...
| 日期 | 变更摘要 |
|------|---------|
| 2026-07-27 | 更新：补充 `classification` deeplink 的真实承接，并同步 `search` / `ranking` / `classification` 三条发现链路的当前代码事实 |
