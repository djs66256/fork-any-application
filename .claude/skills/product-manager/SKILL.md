...
description: >
  ...
  项目进展管理（跟踪功能层面的规划/开发/完成状态，确保迭代上升、避免冲突）。
...
| **需求拆解** | 模糊需求 → 完整 PRD → 子任务拆分 | 确定要做某个功能后 | 主 agent + subagent | [references/prd-writing.md](references/prd-writing.md) |
...
| `progress.md` | 每个功能当前所处阶段（规划→PRD→开发→完成），含链接 | PRD 完成后、feature-workflow 启动/完成后 |
| `decisions/` | 每次「下一步做什么」分析的决策过程和依据 | 每次路线规划分析后 |
| `prd/<slug>/prd.md` | 单个功能的完整需求文档 | 需求拆解阶段 |
| `prd/<slug>/subtasks.md` | 子任务拆分、每端工时估算、迭代计划 | PRD 完成后 |
...
```
  ...
  用户提出想法     综合分析竞品+wiki    撰写完整 PRD        审查 PRD 完整性      feature-workflow
...
- **交付衔接**：PRD 完成后，引导用户通过 `feature-workflow` skill 启动开发。PRD 中的用户故事和功能详述可直接作为 feature-workflow spec 的输入。
...
| 文件 | 用途 |
|------|------|
| [references/roadmap-planning.md](references/roadmap-planning.md) | 路线规划分析流程：如何综合信息做优先级判断 |
...
| [references/progress-management.md](references/progress-management.md) | 进展管理：状态定义、更新流程、冲突检测 |

### Assets（模板）

| 模板 | 用途 |
|------|------|
| `assets/prd-template.md` | PRD 主文档模板 |
