...
2. 按严重程度排序：高 → 中 → 低
3. **去重前先检查**：如果有历史 review 报告（`code-<platform>-review.md` 已存在），将 subagent 返回的「非首轮验证」结果（来源为「验证-未修复」或「验证-修复不完整」的问题）单独列出，不参与去重
4. 合并同类型新问题并去重（同一文件同一行同一类型的问题合并）
5. 按 `assets/code-review-template.md` 模板格式写入 `docs/specs/<YYYY-MM-dd>-<name>/code-<platform>-review.md`
...
5. **非首轮验证发现的问题**（❌ 未修复 / ⚠️ 修复不完整）→ **最高优先级**，立即修复。这些是上一轮已标记修复但实际未改或改不到位的问题，反映了修复流程本身的质量问题，不可再次出现

修复完成后，在 review 文档的「修复记录」中记录修复内容。

**重要**：修复后必须在 review 报告的对应问题条目后追加状态标记：
- `✅ 已修复` — 问题已解决，无需进一步处理
- `🔄 已修复，待验证` — 问题已修改，需重新审查确认

这样在后续 review 循环中，review subagent 可以看到哪些问题已处理、哪些仍待解决。
...
**非首轮重新审查的特别要求**：
- 重新派发专项 subagent 时，每个 subagent 会在 prompt 中自动执行「非首轮验证」：先读取上一轮的 review 报告，验证所有标记 `✅ 已修复` 的问题是否真正被修改
- Coding agent 在汇总结果时，需将「非首轮验证」发现的问题（假修复/修复不完整）单独列出并优先处理
- 如果同一问题在多轮中都出现「假修复」，说明修复流程存在问题，应暂停自动循环并上报人工

**注意**：coding subagent 在重新审查前，应先读取之前的 review 报告，了解已有问题状态。只修复新增或未修复的问题，不要重复处理已标记 `✅ 已修复` 且经验证确认已修复的问题。
