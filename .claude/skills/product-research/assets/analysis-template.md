...
> 本模板用于采集完成后，主 agent 输出结构化分析结果。
> 使用时将 `{{占位符}}` 替换为实际内容，不适用当前采集的章节标注「本次不适用」。
> 本文件保存为 `.captures/<YYYY-MM-DD>-<name>/analysis.md`。
...
{{列出本次采集中发现的新页面、新路径、新交互入口，供验收阶段追加到 PROGRESS.md。}}

| 发现路径 | 说明 | 页面快照 |
|---------|------|---------|
| {{如 homepage-feed/comments}} | {{点击评论图标后进入了评论列表页}} | `assets/{{文件}}.png` |
