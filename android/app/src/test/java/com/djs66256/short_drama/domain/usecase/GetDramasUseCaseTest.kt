...
class GetDramasUseCaseTest {
    ...
    fun `T-04 invoke delegates to repository and returns its result`() = runTest {
        ...
        coEvery { dramaRepository.getDramas(1, 10) } returns expectedResult

        val result = useCase(1, 10)

        coVerify(exactly = 1) { dramaRepository.getDramas(1, 10) }
        assertEquals(expectedResult, result)
    }

    @Test
    fun `T-04 invoke uses canonical default page size when none provided`() = runTest {
        coEvery { dramaRepository.getDramas(1, 10) } returns ApiResult.Success(emptyList())
